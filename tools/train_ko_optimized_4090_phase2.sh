...
# Phase 2: 안정화 및 일반화 (Step 240k → 끝)
#
# 과학적 근거 (이전 학습 데이터 분석 기반):
# - LR: 5e-6 (안정적, 안정화 및 일반화)
# - Batch: 8 (RTX 4090 24GB 검증된 안전값)
# - Grad Accumulation: 8 (effective batch 64, 안정화)
# - Warmup: 1,000 steps
# - Grad Clip: 0.5 (낮은 LR에 맞춘 보수적 안전장치)
#
# 이전 학습 결과 (GRAD_ACC=8, LR=5e-6):
#   - Step 247800: Loss=0.9648 (최저점 달성!)
#   - Step 250k 이후: Overfitting 시작
#
# Phase 2 목표: Step 240k에서 Loss < 1.0 달성
# Phase 2 전환: Step 240k에서 GRAD_ACC=8, LR=5e-6으로 안정화
#
# 예상 학습 시간: ~20시간 (240k steps, GRAD_ACC=8로 빠름)
#
# 주의: Cross-lingual fine-tuning (English base → Korean)
#       Phase 2: 안정화 및 일반화 (현재)
...
echo "📍 Phase 2: 안정화 및 일반화 (Step 240k → 끝)"
echo "================================================================"
echo ""
echo "📊 Phase 2 하이퍼파라미터:"
echo "  - GPU: RTX 4090 24GB"
echo "  - Batch Size: 8"
echo "  - Gradient Accumulation: 8 (안정화)"
echo "  - Learning Rate: 5e-6 (안정적)"
echo "  - Warmup Steps: 1,000"
echo "  - Epochs: 3"
echo "  - Gradient Clip: 0.5 (보수적 안전장치)"
echo ""
echo "💡 2단계 학습 전략:"
echo "  - Phase 2 (0→240k): GRAD_ACC=8, LR=5e-6 (안정화 및 일반화)"
...
# 사용자 확인 스킵 (자동 실행)
echo "자동으로 Phase 2 학습을 시작합니다..."

echo ""
echo "🎬 Phase 2 학습을 시작합니다..."
echo "📊 TensorBoard: http://localhost:6006"
echo "📁 체크포인트: /mnt/sda1/models/index-tts-ko/checkpoints/"
echo ""
echo "⚡ Phase 2 전략:"
echo "   - GRAD_ACC=8로 빠른 학습"
...
# Phase 2: 안정화 및 일반화 (Step 240k → 끝)
# Phase 1에서 이어받기:
#   - Phase 1: GRAD_ACC=1, LR=2e-5로 빠른 수렴
#   - Phase 2: GRAD_ACC=8, LR=5e-6으로 안정화
#   - Step 240k checkpoint부터 재개
#   - Overfitting 방지 및 일반화 성능 향상
SKIP_DATA_CHECK=1 \
LR=5e-6 \
WARMUP_STEPS=1000 \
BATCH_SIZE=8 \
...
MAX_STEPS=0 \
EPOCHS=3 \
BASE_CHECKPOINT="/mnt/sda1/models/index-tts-ko/checkpoints/model_step240000.pth" \
